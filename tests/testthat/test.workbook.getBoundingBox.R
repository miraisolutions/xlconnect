test_that("test.workbook.getBoundingBox", {
    ...
    expect_equal(dim1, res)
    res <- getBoundingBox(wb.xls, sheet = "Test5", startRow = 17, 
        startCol = 7, endRow = 24, endCol = 9)
    expect_equal(dim2, res)
    res <- getBoundingBox(wb.xlsx, sheet = "Test5", startRow = 17, 
        startCol = 7, endRow = 24, endCol = 9)
    expect_equal(dim2, res)
    res <- getBoundingBox(wb.xls, sheet = "Test5", startCol = 7, 
        endCol = 9)
    expect_equal(dim3, res)
    res <- getBoundingBox(wb.xlsx, sheet = "Test5", startCol = 7, 
        endCol = 9)
    expect_equal(dim3, res)
    ...
    expect_equal(dim4, res)
    res <- getBoundingBox(wb.xls, sheet = c("Test1", "Test4", 
        "Test5"))
    expect_equal(dim5, res)
    res <- getBoundingBox(wb.xlsx, sheet = c("Test1", "Test4", 
        "Test5"))
    expect_equal(dim5, res)
})
