test_that("test.writeAndReadNamedRegion", {
    wb.xls <- loadWorkbook(rsrc("resources/testWriteAndReadNamedRegion.xls"), 
        create = TRUE)
    wb.xlsx <- loadWorkbook(rsrc("resources/testWriteAndReadNamedRegion.xlsx"), 
        create = TRUE)
    testDataFrame <- function(wb, df, lref) {
        ...
        createName(wb, name = namedRegion, formula = paste(namedRegion, 
            lref, sep = "!"), worksheetScope = namedRegion)
    ...
    testDataFrameNameScope <- function(wb, df, lref) {
        ...
        createName(wb, name = namedRegion, formula = paste(namedRegion, 
            lref, sep = "!"), worksheetScope = worksheetScopeName)
    ...
    testDataFrameGlobalAndScoped <- function(wb, df_global, df_scoped, 
        ...
        namedRegion <- paste0(deparse(substitute(df_global)), 
            "expect_global")
        ...
        createName(wb, name = namedRegion, formula = paste(sheetNames, 
            lref, sep = "!"), worksheetScope = scopeSheets)
        writeNamedRegion(wb, dfs, name = namedRegion, worksheetScope = scopeSheets, 
            header = TRUE)
    ...
    if (getOption("FULL.TEST.SUITE")) {
        ...
        testDataFrameGlobalAndScoped(wb.xlsx, swiss, morley, 
            "$M$2")
    }
    cdf <- data.frame(Column.A = c(1, 2, 3, NA, 5, Inf, 7, 8, 
        NA, 10), Column.B = c(-4, -3, NA, -Inf, 0, NA, NA, 3, 
        4, 5), Column.C = c("Anna", "???", NA, "", NA, "$!?&%", 
        "(?2@?~?'^*#|)", "{}[]:,;-_<>", "\\sadf\n\nv", "a b c"), 
        Column.D = c(pi, -pi, NA, sqrt(2), sqrt(0.3), -sqrt(pi), 
            exp(1), log(2), sin(2), -tan(2)), Column.E = c(TRUE, 
            TRUE, NA, NA, FALSE, FALSE, TRUE, NA, FALSE, TRUE), 
        Column.F = c("High", "Medium", "Low", "Low", "Low", NA, 
            NA, "Medium", "High", "High"), Column.G = c("High", 
    ...
    writeNamedRegion(wb.xls, mtcars, name = "rownames", header = TRUE, 
        rownames = "Car")
    ...
    writeNamedRegion(wb.xlsx, mtcars, name = "rownames", header = TRUE, 
        rownames = "Car")
    ...
    writeNamedRegion(wb.xls, mtcars, name = "rownames2", header = TRUE, 
        rownames = "Car")
    ...
    writeNamedRegion(wb.xlsx, mtcars, name = "rownames2", header = TRUE, 
        rownames = "Car")
