...
\begin{itemize}
  \item{Fix documentation with respect to conversion of numbers to dates}
  \item{Fixed a bug that prevented rownames to be specified as a list
    in writeWorksheet}
  \item{Automatically adjust for 1904 windowing (thanks to Jennifer Rogers!)}
  \item{Moved to Apache POI 3.11-beta1}
  \item{Added inst/COPYRIGHTS file}
  \item{Added java/README providing information on where XLConnect sources can be
    obtained}
  \item{Split package again into XLConnect and XLConnectJars as requested by CRAN}
\end{itemize}
