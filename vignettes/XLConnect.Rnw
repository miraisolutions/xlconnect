...
\begin{itemize}
  \item{Moved to Apache POI 3.11}
  \item{Fixed issue where milliseconds are getting lost when converting date/time
    values to string (\#28)}
  \item{Fixed problem with non-character column headers}
  \item{Fixed declaration of encoding in vignette}
  \item{Unit test functionality is not part of the public API anymore}
\end{itemize}
