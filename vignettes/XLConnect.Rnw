...
\begin{itemize}
  \item{Moved to Apache POI 3.16}
  \item{Performance improvements to setCellStyle for large row/col vectors}
  \item{Added support for setting hyperlinks}
  \item{Fixed issue with missing classes when opening password protected files (\#61)}
\end{itemize}
