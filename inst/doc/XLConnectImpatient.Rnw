...
\monthname, \the\year \\[15pt]
\end{center}

\begin{tabbing}
\textbf{\underline{Installation:}} ~~~~~~~~~~\= \code{install.packages("XLConnect")}, Mac: \code{install.packages("XLConnect", type = "source")} \\[3pt]
\textbf{\underline{Loading library:}} \> \code{library(XLConnect)} \\
\end{tabbing}
