...
\code{loadWorkbook()} loads a Microsoft Excel workbook, so that it can then be further manipulated. 
...
\subsubsection{Writing into a sheet within an Excel file}
...
writeWorksheet(object, data, sheet, startRow, startCol, header)
\end{tikzCodeBlock}
...
\subsection{Example: writing a sheet}\label{sec:options}
...
<<simpleEx,echo=T,results=hide>>=
require(XLConnect)
wb = loadWorkbook("XLConnectExample2.xlsx", create = TRUE)
createSheet(wb, name = "chickSheet")
...
\begin{figure}[htb]
\begin{center}
\caption{chicks named region written into the XLConnectExample2.xlsx file}
\includegraphics[width=11cm]{img/XLConnectExample2.png}
\end{center}
\end{figure}
