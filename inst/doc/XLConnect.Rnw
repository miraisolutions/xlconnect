...
\code{writeWorksheetToFile()} loads the workbook, creates the sheet within in and saves the workbook. When you only need 
...
\subsection{Example: writing a named region}\label{sec:options}

In this example we will show how to write data into an Excel named region. Again, we will use the
{\bf ChickWeight} dataset.

Similarly as in the example of writing an Excel sheet, we first load the workbook, using \code{loadWorkbook()}, 
and then create a sheet named chickSheet, using \code{createSheet()}.

We then use \code{createName()} to produce a named region "chicks", starting in the C5 cell of the chickSheet.
Calling \code{writeNamedRegion()} writes the name into the workbook. 

At the end, we use \code{saveWorkbook()} to write the Excel file to disk.
...
createSheet(wb, name = "chickSheet")
createName(wb, name = "chicks", formula = "chickSheet!$C$5", overwrite = TRUE)
writeNamedRegion(wb, ChickWeight, name = "chicks")
saveWorkbook(wb)
@
...
\begin{figure}[htb]
\begin{center}
\caption{chicks named region written into the XLConnectExample3.xlsx file}
\includegraphics[width=11cm]{img/XLConnectExample3.png}
\end{center}
\end{figure}

\subsection{Example: writing a named region with one call}\label{sec:options}
...
<<simpleEx,echo=T,results=hide>>=
require(XLConnect)
writeNamedRegionToFile("XLConnectExample4.xlsx", ChickWeight, name = "chicks", 
		formula = "chickSheet!$C$5")
@
...
\code{readNamedRegionFromFile()} is a conveniece wrapper function which allows for reading named regions
...
In this example, we will show how you can use \code{XLConnect} to read from an Excel sheet. For this purpose,
...
For this example we are going to usage some further dependecies (list the packages used, with versions)

\section{What's new in XLConnect version \Sexpr{getOption("XLConnect.Version")}}
\subsection{xlsEdit()}

\pkg{XLConnect} version \Sexpr{getOption("XLConnect.Version")} contains a new function: \code{xlcEdit()}

\code{xlcEdit()} allows you to do the following:

\begin{enumerate}
	\item Write a data frame from R into Excel
	\item Edit the data frame in Excel
	\item Get the updated data back into R
\end{enumerate}

\textcolor{red}{Add an example, discuss with Martin}
...
Please mail any bug reports, feature requests or technical inquiries to: \\
\urlwofont{mailto:xlconnect-bugs@mirai-solutions.com}
\end{minipage}
...
\end{minipage}
\begin{minipage}[c]{0.9\textwidth}
For any other feedback you may have, please contact as at: \\
\urlwofont{mailto:xlconnect@mirai-solutions.com}
\end{minipage}
...
\begin{enumerate}
	...
	\item 	R Base course by Dr. Roberto Dantas de Pinho, referring to \pkg{XLConnect} (Portugese):
	...
	\item 	\pkg{XLConnect} in RStudio:
			\\\urlwofont{http://wiki.alpine-geckos.at/index.php/RStudio}			
