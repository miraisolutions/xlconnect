...
\begin{itemize}
	\item{Added new methods for clearing sheets, named regions and cell ranges}
	\item{Fixed an issue with overwriting cells of a different type}
	\item{Fixed issue with failing unit tests on R-devel (issue with Excel date/time values and increased precision of
...
\begin{enumerate}
	...
	\item 	"Grundlagen der Datenanalyse mit R: Eine anwendungsorientierte Einf?hrung", book by Daniel Wollschl?ger:
