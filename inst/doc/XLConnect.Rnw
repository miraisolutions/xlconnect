...
We then use \code{createName()} to produce a named region "womekName", starting in the C5 cell of the womenData sheet.
...
\newpage\section{What's new in XLConnect version \Sexpr{getOption("XLConnect.Version")}}
\subsection{xlsEdit()}
