...
{\bf edit(file = system.file("XLConnect.R", package = "XLConnect"))}
}
...
With version \Sexpr{getOption("XLConnect.Version")} of \pkg{XLConnect} we moved to Apache POI 3.8-beta5. The
most prominent new features/extensions are an improved vignette (this document!) which provides more information
and examples to get people started with the package. A new type conversion mechanism was introduced which allows
users to pre-specify column types/classes and even force conversions between types (e.g. from \code{character} to
\code{numeric}) when reading in data using \code{readNamedRegion} or \code{readWorksheet}. Instead of relying
completely on cell types (which is the default mechanism), this allows to specify column classes of the data to
be read in upfront. Possible type conversion issues will be reported back to the user/developer via warnings referring
to the actual cell that is causing problems. Another new feature that was introduced is the function \code{xlcEdit}.
It allows to edit plain \code{data.frame}s in an Excel file editor (e.g. MS Excel). Changes will be applied to the
underlying data object after closing the editor. Note that this functionality is only available under Windows and
Mac OS X. For more information please see the \href{http://cran.r-project.org/web/packages/XLConnect/XLConnect.pdf}{Reference Manual}.\\\\
...
\begin{enumerate}
	...
	\item 	How to Read Data from Excel into R by Joris Meys and Andrie de Vries:
