...
\begin{itemize}
	\item{Moved to Apache POI 3.8}
	\item{Fixed lazy evaluation issues with S4 generics. Generic methods now fully expand the argument list in order to have the arguments immediately evaluated.}	
	\item{\code{readWorksheet} now returns a named list if more than one worksheet is read. This provides convenient access to the corresponding data by list subsetting.}
	\item{\code{readWorksheet} and \code{readNamedRegion} now provide a new argument named \code{check.names} that controls whether \code{data.frame} column names are checked for syntactically valid variable names (see the \code{check.names} argument of \code{data.frame}).}	
 	\item{Support for setting the color of worksheet tabs via the \code{setSheetColor} method (only for .xlsx files).} 	
  	\item{Support for freeze panes and split panes.} 
	\item{You can now find all code examples presented in this vignette in the XLConnect.R script file. You can find the file in the top level library directory. Alternatively, you can open the document by typing the following command into your console:
	\\\code{edit(file = system.file("XLConnect.R", package = "XLConnect"))}}
\end{itemize}
...
\begin{enumerate}
	...
	\item 	"Grundlagen der Datenanalyse mit R: Eine anwendungsorientierte Einführung", book by Daniel Wollschläger:
			\\\urlwofont{http://books.google.ch/books/about/Grundlagen_der_Datenanalyse_mit_R.html?id=tlmcPQ9YZP4C&redir_esc=y}
