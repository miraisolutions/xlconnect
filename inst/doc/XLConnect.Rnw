...
The \code{writeWorksheet()} function writes \code{data} into a worksheet (name or index specified as the \code{sheet} argument) 
...
% Shift into the figs directory here so that we don't have
...
Then, via \code{createSheet()}, a sheet named "mtcarsSheet" is created within the workbook. 
We then use \code{writeWorksheet()} to write the {\bf mtcars} data frame into mtcarsSheet.
...
createSheet(wb, name = "mtcarsSheet")
writeWorksheet(wb, mtcars, sheet = "mtcarsSheet", startRow = 3, startCol = 4)
saveWorkbook(wb)
@
...
We are very glad you are using \pkg{XLConnect} and we would be happy to hear from you!\\
